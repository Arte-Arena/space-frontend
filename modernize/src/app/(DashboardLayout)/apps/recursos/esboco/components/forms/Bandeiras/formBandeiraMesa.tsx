...
const FormBandeiraMesa: React.FC<FormProps> = ({ form, handleChange, handleCheckboxChange, setForm }) => {
    ...
    <>
      <Grid container spacing={2} mb={2}>
        ...
        <Grid item xs={12} sm={1.5}>
          <FormControl fullWidth>
            <InputLabel>N° de Hates</InputLabel>
            <Select
              value={form.qntHastes}
              label="qntHastes"
              onChange={handleSelectChange('qntHastes')}
            >
              {['1', '2', '3'].map((qntHastes) => (
                <MenuItem key={qntHastes} value={qntHastes}>
                  {qntHastes}
                </MenuItem>
              ))}
            </Select>
...
export default FormBandeiraMesa;
